...
import { BaseMap, type ClusteredMarker, type FlyToTarget } from '@/features/gis/components/BaseMap'
...
const COLUMNS: DataTableColumn<DashboardAvu>[] = [
  ...
  { key: 'descricao', header: 'Descrição', render: (avu) => <span className="line-clamp-1 max-w-xs">{avu.descricao}</span> },
...
export function MapPage() {
  ...
  const avusQuery = useQuery({ queryKey: ['map', filters], queryFn: () => listAvusForDashboard(filters) })
  ...
  const clusteredMarkers: ClusteredMarker[] = useMemo(
    () =>
      avus.flatMap((avu) => {
        ...
        if (!colorInfo) return []
        return [{ id: avu.id, longitude: avu.longitude, latitude: avu.latitude, color: colorInfo.color }]
      }),
    [avus],
  )
      ...
      <Card>
        <CardContent className="p-0">
          <DataTable
            ...
            getRowClassName={(avu) => (avu.id === selectedAvuId ? 'bg-primary-50' : undefined)}
          />
        </CardContent>
      </Card>

      {selectedAvu && <AvuMapPanel avu={selectedAvu} onClose={() => setSelectedAvuId(null)} />}
    </div>
  )
}
