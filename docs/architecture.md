...
O pipeline (PDF → extração de texto digital, com fallback de OCR via `lib/ocr.ts` se o texto for insuficiente → extração de campos → extração de imagens → classificação IA → validação → criação do AVU) roda inteiro dentro da function, autenticado com o JWT de quem chamou (a mesma disciplina das RPCs `security definer`: a function nunca confia só em quem conseguiu invocá-la). A AVU real só é criada em `avus` quando a validação passa (confiança ≥ 80%) ou quando um humano confirma na tela de revisão (`pages/ImportReviewPage.tsx`) — os arquivos (PDF + imagens extraídas, com miniaturas visíveis já na revisão) ficam num bucket de staging (`avu-import-staging`) até esse momento, e o PDF final + imagens extraídas viram `avu_attachments` normais (mesmo bucket/tabela da Sprint 2).

Validado de ponta a ponta contra um PDF real de produção (não apenas escrito especulativamente — ver `docs/testing.md`, seção "causa raiz do bug relatado", para os detalhes técnicos de extração de texto/imagem que só se confirmam rodando contra um arquivo real). `extractFields.ts` assume o formato real confirmado — rótulo numa linha, valor na linha seguinte, sem dois-pontos — não "Rótulo: valor" numa única linha. Extração de imagem sempre re-codifica os pixels decodificados como PNG (`lib/pngEncoder.ts`, sem dependência externa), já que a lib de PDF nunca entrega os bytes originais do stream.
